...
(function ($) {
	...
	HighlightWithinTextarea.prototype = {
		...
		generate: function () {
			...
			// .on('keydown.' + ID, this.handleInput.bind(this));
			// if(lineHeight === 'normal'){
			// 	lineHeight = "18px";
			// }
			this.$highlights = $('<div>', { 
				class: ID + '-highlights ' + ID + '-content hwtTA',
			});
			...
			this.$backdrop = $('<div>', { class: ID + '-backdrop hwtTA' })
				// .attr('style', elStyles)
				.css({ height: height + 'px', width: width + 'px', padding })
				.append(this.$highlights);

			this.$container = $('<div>', { class: ID + '-container hwtTA' })
